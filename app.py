...
app.py
------
Streamlit front-end for the IEEE RAS RAG assistant.

Run locally with:  streamlit run app.py

NOTE: st.set_page_config() must be the very first Streamlit call in the file.
Nothing that touches Streamlit may run above it.
"""
...
# Streamlit Cloud wipes the filesystem on every restart, so the vector database
# has to be rebuilt on first load. @st.cache_resource makes this run ONCE per
# server process instead of on every rerun (every click would be far too slow).
@st.cache_resource(show_spinner="Setting up the knowledge base (first load only)...")
def bootstrap_knowledge_base():
    import ingest
...
try:
    CHUNK_COUNT = bootstrap_knowledge_base()
    BOOTSTRAP_ERROR = None
except Exception as exc:  # show the problem instead of a red traceback
    CHUNK_COUNT = 0
    BOOTSTRAP_ERROR = exc

from rag_engine import answer_question  # noqa: E402  (must come after bootstrap)
...
def render_extras(message):
    """Show source citations and the expandable retrieved-context panel."""
    ...
    if message.get("chunks"):
        with st.expander("Show retrieved context"):
            for chunk in message["chunks"]:
                st.markdown(
                    f"**{chunk['source']}** — distance {chunk['distance']:.3f}"
                )
                preview = chunk["text"][:500]
                st.text(preview + ("..." if len(chunk["text"]) > 500 else ""))
...
# A suggested-question click takes priority over the text box
prompt = st.session_state.pop("pending_question", None)
typed = st.chat_input("Ask a question about IEEE RAS...")
if typed:
    prompt = typed
